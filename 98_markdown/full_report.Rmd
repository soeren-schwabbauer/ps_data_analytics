...
The graphs indicate a first rough overview of the political climate in Romania. For more than half of the people are not very interested to not at all interested in politics, which reflects in taking politial actions. Only few people have signed a petition, joined a boycott, joined a lawful demonstration or even joined an unofficial strike. Nevertheless, a slight majority sees democracy as a very good idea, and highlight the importance of democracy.

The following tables provide an overview of the exact scaling for the variables
...
Looking at the principal components and their visualization within the screeplot it is tricky to clearly identify the number of PCs that should be considered. There is no clearly visible “elbow” which would identify the number of components that need to be considered to cover most of the variance. However, considering the cumulative proportion within the summary of the PCA the analysis manages to explain 82.60% of variance when using 5 PCs. Therefore, moving forward, the analysis will consider 5 principal components.
...
In the Biplot between the first and the second principal component, we can see that the importance of democracy is negatively correlated to the attitude of having a democratic system. Due to the opposite scaling, it can be seen, that seeing the democracy as important also induced a positive attitude towards the democratic system. 

The remaining vectors rather form a 90 degree angle towards the two mentioned vectors. Therefor it can be said, that the importance of democracy or the satisfaction with the democratic system is uncorrelated to whether somebody has attended a boycot, a strike, or a demonstration.
...
What sticks out in comparing the first and fourth principal component, is that a higher importance in democracy seems to be negatively correlated to the attendance of an unlawful strike. Here it can also be seen, that unlawful strikes and attending a demonstration are two different things. Meaning, that a demonstration is rather attended by people who have a high importance in democracy.


</div>
___ 

Starting off, those variables closely monitoring active (often activist) means of political participation are closely related to one another. More concretely, we can see in almost all biplots, that politics_petition, politics_boycott, politics_demo and politics_strikes move in very similar directions and can therefore be understood as being correlated to a certain extent.


Interpreting the biplots is a little more tricky since when doing so, one needs to take into account how the data is scaled. For instances, the scaling of the data will be specifically noted. However, looking at the biplots between PC 1 and 2 as well as 1 and 3 a couple things become clear which are very interesting to note. 



Another interesting connection is between the variables politics_democracy and importance_democracy. On the biplots these two seem to contradict each other. However, this is one of the instances where the scaling of the variables is important. politics_democracy measures a given respondent's opinion on living in a democratic system. This is measured on a scale from "very good" to "very bad" but it is important to note, that the numeral value of 1 implies a positive opinion on living in a democracy whereas the value 4 implies the opposite. Looking at importance_democracy, this measures the importance, that a given respondent attributes to the concept of democracy as a whole. This is measured on a scale from 1-10 with 10 attributing the highest level of importance to the concept of democracy. 

That being said, it makes sense, that a respondent who thinks highly of democracy would attribute a lot of importance to the concept of democracy while simultaneously being convinced that living in a democracy is an important thing. And looking at the scaling of these answers it makes sense, that on the biplots, they seem to contradict one another. Since a higher value in importance_democracy seems to correlate with a lower value in politics_democracy.
